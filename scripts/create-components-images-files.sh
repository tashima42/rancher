...
if [ ${ARCH} == amd64 ]; then
  ...
  go run ../cmd/images/main.go \
    --tag "$TAG" \
    --charts-path "$CHARTS_DIRS" \
    --extra-images "$IMAGE,$AGENT_IMAGE,$SYSTEM_AGENT_UPGRADE_IMAGE,$WINS_AGENT_UPGRADE_IMAGE,$CLUSTER_API_CONTROLLER_IMAGE,$(printf "%s," "${SYSTEM_AGENT_INSTALLER_RKE2_IMAGES[@]}" | sed 's/,$//'),$(printf "%s," "${SYSTEM_AGENT_INSTALLER_K3S_IMAGES[@]}" | sed 's/,$//')"
fi
